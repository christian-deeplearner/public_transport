...
class Line:
    ...
    def _handle_arrival(self, message):
        ...
        if prev_dir is not None and prev_station_id is not None:
            ...
            logger.debug(
                "unable to handle previous station due" +
                "to missing previous info"
            )
    ...
    def process_message(self, message):
        ...
        if message.topic() == "org.cta.chicago.stations.table.v1":
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message.topic().startswith("org.cta.chicago.station.arrivals"):
